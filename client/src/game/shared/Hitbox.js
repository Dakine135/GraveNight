...
exports.create = ({ id = 0, x = 0, y = 0, width = 50, height = 50, angle = 0 }) => {
    let top = y - height / 2;
...
function getVisualPoints({ obj, viewPoint, getPointsAfterEdge = false }) {
    let returnPoints = [];
    if (viewPoint.y < obj.top) {
        //NW, N, NE
        if (viewPoint.x < obj.left) {
            //NW
    ...
    } else if (viewPoint.y > obj.bottom) {
        //SW, S, SE
        if (viewPoint.x < obj.left) {
            //SW
...
function colliding(obj1, obj2) {
    ...
    if (hitbox1.top > hitbox2.bottom || hitbox1.bottom < hitbox2.top || hitbox1.right < hitbox2.left || hitbox1.left > hitbox2.right) {
